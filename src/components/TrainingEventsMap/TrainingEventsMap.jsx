  ...
  return (
    ...
    >
      {markers.length === 0 ? (
        ...
        >
          <Typography>No event locations with coordinates to show on the map.</Typography>
        </Box>
      ) : (
        <MapContainer
        ...
        >
          ...
          {markers.map((marker) => (
            <Marker key={marker.id} position={marker.position} icon={createGreenMarker()}>
              <Popup>
                <Box sx={{ minWidth: "200px", maxWidth: "320px" }}>
                  <Box sx={{ display: "flex", alignItems: "center", gap: 0.5, mb: 1 }}>
                    <LocationOn sx={{ fontSize: 16, color: "#17cf54" }} />
                    <Typography variant="subtitle2" sx={{ fontWeight: 700, color: "#17cf54", textTransform: "uppercase", fontSize: "0.75rem" }}>
                      {marker.location}
                    </Typography>
                  </Box>
                  <Typography variant="subtitle1" sx={{ fontWeight: 700, mb: 0.5, color: "#0e1b12", fontSize: "1rem" }}>
                    {marker.title}
                  </Typography>
                  {marker.type && (
